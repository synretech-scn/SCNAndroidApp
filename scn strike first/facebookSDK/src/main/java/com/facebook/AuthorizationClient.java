...
class AuthorizationClient implements Serializable {
    ...
    void startOrContinueAuth(AuthorizationRequest request) {
        if (appEventsLogger == null
                || appEventsLogger.getApplicationId() != request
                .getApplicationId()) {
                ...
                try {
                    ...
                    if (result != null) {
                        ...
                        if (data != null && data.size() == 1) {
                            ...
                            tokenPermissions.addAll(permissions.asMap()
                                    .keySet());
                        }
                    }
                } catch (Exception ex) {
    ...
    private void logAuthorizationMethodStart(String method) {
        ...
        appEventsLogger
                .logSdkEvent(EVENT_NAME_LOGIN_METHOD_START, null, bundle);
    }
    ...
    private void logAuthorizationMethodComplete(String method, String result,
        ...
        appEventsLogger.logSdkEvent(EVENT_NAME_LOGIN_METHOD_COMPLETE, null,
                bundle);
    }
    ...
    class KatanaLoginDialogAuthHandler extends KatanaAuthHandler {
        ...
        boolean tryAuthorize(AuthorizationRequest request) {
            ...
            Intent intent = NativeProtocol.createLoginDialog20121101Intent(
                    context, request.getApplicationId(), new ArrayList<String>(
                            request.getPermissions()), request
                            .getDefaultAudience().getNativeProtocolAudience());
            if (intent == null) {
                return false;
            }
            ...
            addLoggingExtra(
                    EVENT_EXTRAS_PROTOCOL_VERSION, "" + (intent.getIntExtra(NativeProtocol.EXTRA_PROTOCOL_VERSION, 0)));
            addLoggingExtra(
                    EVENT_EXTRAS_PERMISSIONS,
                    TextUtils.join(
                            ",",
                            intent.getStringArrayListExtra(NativeProtocol.EXTRA_PERMISSIONS)));
            addLoggingExtra(EVENT_EXTRAS_WRITE_PRIVACY,
                    intent.getStringExtra(NativeProtocol.EXTRA_WRITE_PRIVACY));
            logEvent(AnalyticsEvents.EVENT_NATIVE_LOGIN_DIALOG_START,
                    AnalyticsEvents.PARAMETER_NATIVE_LOGIN_DIALOG_START_TIME,
                    callId);
        ...
        boolean onActivityResult(int requestCode, int resultCode, Intent data) {
            ...
            logEvent(
                    AnalyticsEvents.EVENT_NATIVE_LOGIN_DIALOG_COMPLETE,
                    AnalyticsEvents.PARAMETER_NATIVE_LOGIN_DIALOG_COMPLETE_TIME,
                    callId);
        ...
        private Result createCancelOrErrorResult(AuthorizationRequest request,
                                                 Intent data) {
            Bundle extras = data.getExtras();
            String errorType = extras
                    .getString(NativeProtocol.STATUS_ERROR_TYPE);
            ...
            if (NativeProtocol.ERROR_USER_CANCELED.equals(errorType)
                ...
                String errorJson = extras
                        .getString(NativeProtocol.STATUS_ERROR_JSON);
                String errorCode = null;
                if (errorJson != null) {
                    try {
        ...
        private void logEvent(String eventName, String timeParameter,
                              String callId) {
            if (callId != null) {
                AppEventsLogger appEventsLogger = AppEventsLogger.newLogger(
                        context, applicationId);
                Bundle parameters = new Bundle();
                parameters.putString(AnalyticsEvents.PARAMETER_APP_ID,
                        applicationId);
                parameters.putString(AnalyticsEvents.PARAMETER_ACTION_ID,
                        callId);
